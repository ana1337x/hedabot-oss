...
A Discord bot that joins your voice channel, transcribes everything said in real time, posts each line to a text channel, and saves the full transcript to Supabase.

Transcription is powered by [Deepgram](https://deepgram.com). Works with Discord's DAVE voice encryption (mandatory since March 2026).
...
```bash
git clone https://github.com/your-username/hedabot-oss
cd hedabot-oss
cp .env.example .env
...
First time? Read the [full setup guide](SETUP.md) — it walks through every step.
...
Your Supabase project ends up with a `meetings` table and a `transcripts` table you can query, export, or build on top of however you like.
